...
function restore(): { texts: [string, string]; names: [string, string]; theme: Theme } {
  try {
    const value = JSON.parse(localStorage.getItem('diffchecker-session') || 'null');
    if (value && Array.isArray(value.texts) && value.texts.length === 2 && value.texts.every((v: unknown) => typeof v === 'string')) return { texts: value.texts, names: Array.isArray(value.names) && value.names.length === 2 && value.names.every((v: unknown) => typeof v === 'string') ? value.names : ['original.txt', 'modified.txt'], theme: ['system', 'light', 'dark'].includes(value.theme) ? value.theme : 'system' };
  } catch { /* Storage may be unavailable. */ }
  return { texts: [original, modified], names: ['binary-search.ts', 'binary-search.ts'], theme: 'system' };
}
...
function App() {
  ...
  const [texts, setTexts] = useState<[string, string]>(initial.texts);
  const [names, setNames] = useState<[string, string]>(initial.names);
  const [theme, setTheme] = useState<Theme>(initial.theme);
  const [systemDark, setSystemDark] = useState(matchMedia('(prefers-color-scheme: dark)').matches);
  const [wrap, setWrap] = useState(true);
  const [collapse, setCollapse] = useState(false);
  const [language, setLanguage] = useState(initial.names[0].endsWith('.ts') ? 'TypeScript' : 'Plain text');
  ...
  useEffect(() => { const id = setTimeout(() => { try { localStorage.setItem('diffchecker-session', JSON.stringify({ texts, names, theme })); } catch { setToast('Session could not be saved on this device.'); } }, 500); return () => clearTimeout(id); }, [texts, names, theme]);
  ...
  return <div className="flex h-dvh min-h-[500px] min-w-[680px] flex-col overflow-hidden">
    ...
    <main className="flex min-h-0 flex-1 gap-4 px-5 pb-5">
      ...
      <section aria-label="Text comparison editors" className="flex min-w-0 flex-1 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_2px_8px_0_#00000003] dark:border-white/10 dark:bg-[#1b1e25]">
        ...
        <DiffEditor ref={editor} initial={initial.texts} onChange={(a, b, count) => { setTexts([a, b]); setChunks(count); }} wrap={wrap} collapse={collapse} language={language} dark={dark} />
    ...
    <footer className="flex h-8 shrink-0 items-center justify-between border-t border-slate-200/70 px-6 text-[10px] text-slate-400 dark:border-white/[0.07]"><span className="flex items-center gap-1.5"><span className="h-1 w-1 rounded-full bg-emerald-500" />All changes saved on this device</span><span className="flex items-center gap-1.5"><Keyboard size={12} />⌘ / Ctrl + F to find<span className="mx-2 opacity-40">|</span>DiffChecker v0.1</span></footer>
